...
<h2 class="main-content-title">
	<span>Choisissez quel enfant participera</span>
</h2>
<div class="wrapper radio" id="listChild">

	<?php foreach (PERSON_CONNECTED['children'] as $child) : ?>
		<?php $currentAge = showAge($child['birthdate'], false); ?>
		<?php $ages[] = $currentAge; ?>

        <div class="item childSelected" data-child-age="<?= $currentAge; ?>" data-child-id="<?= $child['childId']; ?>" data-child-name="<?= $child['firstname']; ?>" onclick="changeChild(this)">
...
<div class="form-submit btn-primary">
	<input type="button" class="btn-primary with-icon" onclick="addToList()" value="Ajouter au panier">
</div>
...
if ($params->isSportSelectable == false) :
	foreach ($params->sports as $sport) :
		...
		$listSportName = $listSport . $sport->name . ',';
	endforeach;
	$listSport = substr($listSport, 0, -1);
	$listSportName = substr($listSportName, 0, -1);
endif;
...
<input type="hidden" id="sportsName" value="<?= $listSportName; ?>">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			setTimeout(function() {
				...
				var child = localStorage.getItem('aLaCarteChild');
				var sports = localStorage.getItem('aLaCarteSport');
				...
				if (splitSport.length >= 1) {
					...
					splitSport.forEach(function(sportData, i = 0) {
						...
						$('[data-sport="' + sportData + '"]').trigger('click');

					});


				} else {
					$('[data-sport="' + sports + '"]').trigger('click');
				}
				...
				if (splitChild.length >= 1) {
					...
					splitChild.forEach(function(childData, i = 0) {
						$('[data-child-id="' + childData + '"]').trigger('click');

					});


				} else {
					$('[data-child="' + child + '"]').trigger('click');
				}
		...
		$('.item').click(function() {
			...
			if ($(this).hasClass('active')) {
				...
				if ($(this).parent().hasClass('radio')) {
					...
					$(this).children('.close').fadeOut(100);
					$(this).addClass('active').addClass('margin');
					$('.close', this).delay(100).fadeIn(100);
		...
		var changeChild = (data) => {
			...
			setTimeout(function() {
				...
				$("#listChild")
					...
					.each(function() {
						...
						if (listChild != '') {
							...
							listChildName = listChildName + ', ' + $(this).attr('data-child-name');
						} else {

							listChild = $(this).attr('data-child-id');
							listChildName = $(this).attr('data-child-name');
				...
				$("#childName").val(listChildName);
				$("#child").val(listChild);


				if (i == 0 || i == 1) {
					$(".amount").html(priceBase);
					$("#priceTotal").val(priceBase);
				} else {
				...
				$("#listSports")
					...
					.each(function() {
						...
						if (listSport != '') {
							...
							listSportName = listSportName + ', ' + $(this).attr('data-sport-name');
						} else {
							listSport = $(this).attr('data-sport');
							listSportName = $(this).attr('data-sport-name');
						}



					});
				$("#sportsName").val(listSportName);
